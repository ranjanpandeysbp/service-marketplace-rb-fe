...
const Provider = () => {
    ...
    const setUserSubCategoryChoice = (choice) => {
    ...
    const onSubmit = async (event) => {
        ...
        let request = {
            title, description, mobile, altNumber, email1, email2, operationTiming,
            categoryId, subCategoryId, addressLine, street,
    ...
    const loadSubCategories = async (categoryId) => {
        try {
            ...
            for (let i = 0; i < resData?.data?.length; i++) {
                itemList.push({ value: resData.data[i].id, label: resData.data[i].name })
            }
            setSubCategories(itemList);
            itemList = [];
            setLoading(false);
        } catch (error) {
            setLoading(false);
    ...
    return (
        <div className="smp-container mx-auto">
            <div className="card-body">
                <h3 className="card-title text-center mb-3">Onboard your business</h3>
                <form id="registrationForm" onSubmit={onSubmit}>
                    ...
                    <div className="row">
                        ...
                        <div className="col-md-6">
                            <div className="mb-3">
                                <label htmlFor="categories" className="form-label">Business category</label>
                                <Select name="name" id="categories" required options={categories} onChange={(choice) => setUserCategoryChoice(choice)} />
                            </div>
                        </div>
                        <div className="col-md-6">
                            <div className="mb-3">
                                <label htmlFor="subcategories" className="form-label">Business sub category</label>
                                <Select name="name" id="subcategories" required options={subCategories} onChange={(choice) => setUserSubCategoryChoice(choice)} />
                            </div>
                        </div>
                    </div>
                    <div className="row">
        ...
        // <div className="smp-container mx-auto mt-5" style={{ marginTop: "100px" }}>
        //     <div className="mt-3 border border-success" data-aos="fade-right">
